...
def webcam_feed():
    ...
    process = Popen(["python", "detect.py", '--source', '0', "--weights","NICbest.pt"], shell=True)
    process.communicate()
    process.wait()
    return render_template('index123.html')
...
def predict_img():
    if request.method == "POST":
        if 'file' in request.files:
            ...
            process = Popen(["python", "detect.py", '--source', filepath, "--weights","NICbest.pt"], shell=True)
            process.wait()
    ...
    return render_template('index123.html', image_path=image_path)
    #return "done"
...
if __name__ == "__main__":
    ...
    model = torch.hub.load('.', 'custom','NICbest.pt', source='local')
    model.eval()
